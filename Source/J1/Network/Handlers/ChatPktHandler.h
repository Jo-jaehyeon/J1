...
public:
	static void Init()
	{
		...
		GPacketHandler[Chat::MessageCode::PKT_RES_ENTER_ROOM] = [](SessionPtr& session, boost::asio::mutable_buffer& buffer, int32& offset) {
			return DispatchPacket<Chat::RES_ENTER_ROOM>(Handle_RES_ENTER_ROOM, session, buffer, offset);
			};
		GPacketHandler[Chat::MessageCode::PKT_RES_CHAT] = [](SessionPtr& session, boost::asio::mutable_buffer& buffer, int32& offset) {
